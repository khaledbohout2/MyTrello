...
class BoardCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    func addnavigationbar(){
        let navBar: UINavigationBar = UINavigationBar(frame: CGRect(x: 20, y: 40, width: 375, height:60))
        self.view.addSubview(navBar);
        let navItem = UINavigationItem(title: "My Trello");
        let addbuttonbar = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(addListTapped(_:)))
        navItem.rightBarButtonItem = addbuttonbar
